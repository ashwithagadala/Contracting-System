function routes(app, web3, Party, Tender, Bid){
    ...
    app.get("/api/tenders/winner",async(req,res,next)=>{
        var bid = await Bid.deployed();
        ...
        .then(async(data)=>{
            ...
            let maxBidAmount = "99999999999999";
            let maxBidder = {};

            bidsList.forEach(bid => {
                if (bid.QuoteAmount < maxBidAmount) {
                    maxBidAmount = bid.QuoteAmount;
                    maxBidder = { ...bid };
                }
            });
            var bid = await Bid.deployed();
            bid.finaliseWinnerBid(req.query.address,req.query.tenderId,maxBidder.BidId, {from:req.query.address})
            .then((data)=>{
                console.log("updaged Bid",data.response);
            })
            .catch(err=>{
                console.log(err)
            })

            res.json({"status":"success","response" : maxBidder})
        })
        .catch(err=>{
