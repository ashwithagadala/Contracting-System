...
export class CustomTableComponent<T> {
  ...
  validateTender(tenderId: any) {
    let address=localStorage.getItem("WALLETID");
    Swal.fire({
      title: "Are you sure to finalize the winner !",
      icon: "warning",
      showCancelButton: true,
      confirmButtonColor: "#3085d6",
      cancelButtonColor: "#d33",
      ...
      if (result.isConfirmed) {
        console.log("yes");
        this.tenderService.getTenderWinner(address as string,tenderId).subscribe((bid : any) => {
          Swal.fire({
            title: `The Winner is ${bid.response.bidderAddress}`,
            icon: "success",
            showCancelButton:true,
